...
# 
# weights reversed as more negatives than positives
# using logloss
...
work_dir="/media/hamel/Workspace/Dropbox/Projects/AD-classification/6.XGBoost/upsampling/AD_classifier_logloss_FINAL/"
...
plot(tune_results_1$evaluation_log$test_logloss_mean, ylim=c(0,1) , xlim=c(0,length(tune_results_9[[eval(tune_results_9_lowest_logloss[1,1])]]$evaluation_log$test_logloss_mean)), type = 'l', col="cyan")
...
pdf("variable_importance_top20.pdf")
ggplot(head(importance, 20), aes(x=reorder(Feature,Gain), y=Gain,fill=Gain))+ 
  geom_bar(stat="identity", position="dodge")+ coord_flip()+
  ylab("Relative importance")+
  xlab("Gene")+
...
ggplot(FINAL_test_prediction_missclassifcation_copy, aes(x=Diagnosis, y=Prediction, color=Diagnosis)) + 
  geom_boxplot() +
  geom_jitter(shape=16, position=position_jitter(0.2)) +
  ggtitle("(a) 'AD vs healthy control' classification model prediction probability") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
dev.off()
...
saveRDS(best_model_ROC_pred, file="AD_best_model_ROC_perf")
...
test<-merge(test, gender, by="sample_ID")
dim(test)
head(test)
...
setwd(work_dir)
#save.image("AD_classifier_logloss.Rdata")
