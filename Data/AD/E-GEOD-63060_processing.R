...
# DATE: 09/05/2018
# ARRAY EXPRESS NUMBER: E-GEOD-63060
# DISORDER: Alzheimer's Disease
# MICROARRAY PLATFORM: Illumina
# EXPRESSION CHIP: HumanHT-12 v3.0
# NUMBER OF SAMPLES: 
# TISSUE: Whole Blood
#
# NOTES - 
# # Gender info in GEO missmatch. Used updated gender from authors
# # diagnosis taken from GEO
#
...
work_dir="/media/hamel/Workspace/Dropbox/Projects/AD-classification/1.Data/1.AD/E-GEOD-63060"
...
#raw data only
#data_raw=getAE("E-GEOD-63060", type = "raw")

# #processed data only
# data_raw=https_getAE("E-GEOD-63060", type = "processed")
# 
# #all data
# #data_raw=https_getAE("E-GEOD-63060", type = "full")
# 
...
# write.csv(pData(expression_data), file="GEO_phenotype.txt")

# setwd(raw_dir)
# 
# expression_data_raw<-read.csv("GSE63060_non-normalized.txt", sep="\t", check.names = F)
# 
# dim(expression_data_raw)
...
##
# 
# phenotype_data2<-pData(expression_data)
# head(phenotype_data2)
# names(phenotype_data2)
# 
# phenotype_data_subset<-phenotype_data2
# head(phenotype_data_subset)

##### ADJUST PHNEOTYPE AND EXPRESSION DATA TO MATCH #####

# downloaded phenotype data from synapse

phenotype_data<-read.csv("/media/hamel/Workspace/Dropbox/Projects/AD-classification/1.Data/1.AD/ANM_pheno_form_Angela/ANM1_Pheno_for_QC.csv")
rownames(phenotype_data)<-phenotype_data$Sample_ID
phenotype_data$Sample_ID<-NULL

head(phenotype_data)

# merge sample id
dim(phenotype_data)
dim(pData(expression_data))

# extract pheno + sample ID
pheno_temp<-pData(expression_data)[c(1,10)]
pheno_temp$Sample_ID<-rownames(pheno_temp)
rownames(pheno_temp)<-pheno_temp$title
pheno_temp$title<-NULL


phenotype_data<-merge(pheno_temp, phenotype_data, by="row.names")
head(phenotype_data)
rownames(phenotype_data)<-phenotype_data$Sample_ID
phenotype_data$Row.names<-NULL
phenotype_data$Sample_ID<-NULL
phenotype_data$Diagnosis<-NULL

# used GEO diagnosis
colnames(phenotype_data)[1]<-"Diagnosis"

head(phenotype_data)
dim(phenotype_data)

#check colnames in pheno data
head((exprs(expression_data)))[1:5]

all(colnames((exprs(expression_data))) %in% rownames(phenotype_data))==T

# subset pheno data to that in expression

all(rownames(phenotype_data) %in% colnames((exprs(expression_data))))==T

##### SUBSET TO SAMPLES OF INTEREST #####

# susbet phnotype to match expression 

# case + control

head(phenotype_data)
table(phenotype_data$Diagnosis)
phenotype_data_subset<-subset(phenotype_data, Diagnosis %in% c("status: AD", "status: CTL"))
dim(phenotype_data_subset)
table(phenotype_data_subset$Diagnosis)

names(phenotype_data_subset)

# extract pheno of interest
Ethnicity<-phenotype_data_subset[4]
colnames(Ethnicity)<-"Ethnicity"

Tissue<-phenotype_data_subset[1]
colnames(Tissue)<-"Tissue"

Age<-phenotype_data_subset[2]
colnames(Age)<-"Age"

Diagnosis<-phenotype_data_subset[1]
colnames(Diagnosis)<-"Diagnosis"

Gender<-phenotype_data_subset[3]
...
table(Diagnosis)
Diagnosis[grep("CTL", Diagnosis[,1]),]<-"control"
Diagnosis[grep("AD", Diagnosis[,1]),]<-"case"
table(Diagnosis)
...
expression_data_raw<-exprs(expression_data)

expression_data_subset<-expression_data_raw[,colnames(expression_data_raw) %in% rownames(phenotype_data_subset)]
...
boxplot(head(expression_data_subset))
png(file="raw_data_boxplot.png")
boxplot(expression_data_subset)
dev.off()

plotDensity(head(as.matrix(expression_data_subset)), logMode=F, addLegend=F)
png(file="raw_data_density_plot.png")
plotDensity(as.matrix(expression_data_subset), logMode=F, addLegend=F)
dev.off()
...
#expression_data_normalised<-rsn(log2(as.matrix(expression_data_subset)))
...
#****** predicted gender for controls seem right according to HK genes - use predicted gender for controls + case
...
pdf("case_females_sample_network_analysis.pdf")
data_case_exprs_good_probes_F_QC<-run_sample_network_plot(data_case_exprs_good_probes_F, sample_network_ZK_threshold)
dev.off()
