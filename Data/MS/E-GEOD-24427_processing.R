...
# DATE: 12/05/2018
# ARRAY EXPRESS NUMBER: E-GEOD-24427
# DISORDER: Multiple Sclerosis
# MICROARRAY PLATFORM: Affy
# EXPRESSION CHIP: HG U133A + HG U113B
# NUMBER OF SAMPLES: 
# TISSUE: Whole Blood
#
# NOTES - 
# single disease (no controls)
# only processing U133A
# exprs from GEO, pheno from ArrayExpress
#
...
work_dir="/media/hamel/Workspace/Dropbox/Projects/AD-classification/1.Data/3.MS/E-GEOD-24427"
...
#raw data only
#data_raw=getAE("E-GEOD-24427", type = "raw")

#processed data only
#data_raw=https_getAE("E-GEOD-24427", type = "processed")

#all data
data_raw=https_getAE("E-GEOD-24427", type = "full")

##### CREATE R EXPRESSION OBJECT #####

# METHOD 1 - convert MAGE-TAB files into expresssion set  - USING RAW DATA
# use to get pheno info
expression_data2 = ae2bioc(mageFiles = data_raw)
...
# 
# #unable to process through array express. used GEO instead
expression_data <- getGEO("GSE24427", GSEMatrix =TRUE, getGPL=FALSE)
...
head(pData(expression_data[[1]]))
head(exprs(expression_data))[,1:5]

table(pData(expression_data[[1]])$Characteristics.edss.)
...
phenotype_data<-pData(expression_data[[1]])
head(phenotype_data)
...
# extract all before treatment
phenotype_data_subset<-phenotype_data[grep("before first", phenotype_data$title),]
table(phenotype_data_subset$title)
phenotype_data_subset
...
Age<-phenotype_data_subset[11]
colnames(Age)<-"Age"

Diagnosis<-phenotype_data_subset[22]
...
table(Age)
Age$Age<-as.numeric(gsub("age at ifn-beta therapy onset \\(baseline blood sampling\\): ", "", Age$Age))
#Age[1]<-"Unknown"
table(Age)
...
#expression_data_subset<-expression_data_subset[,gsub(".CEL", "", colnames(expression_data_subset)) %in% rownames(phenotype_data_subset)]
# 
# expression_data_normalised_as_data_frame<-expression_data_normalised_as_data_frame[,colnames(expression_data_normalised_as_data_frame) %in% rownames(phenotype_data_subset)]
# dim(expression_data_normalised_as_data_frame)
# 
# # check same number of samples
# ncol(expression_data_normalised_as_data_frame)==nrow(phenotype_data_subset)
# ncol(expression_data_normalised_as_data_frame)
# 
expression_data_subset<-expression_data2[[2]][,gsub(".CEL", "", colnames(exprs(expression_data2[[2]]))) %in% rownames(phenotype_data_subset)]
...
# separate by gender
case_exprs_F<-expression_data_normalised_as_data_frame[colnames(expression_data_normalised_as_data_frame)%in%rownames(female_samples)]
case_exprs_M<-expression_data_normalised_as_data_frame[colnames(expression_data_normalised_as_data_frame)%in%rownames(male_samples)]

dim(case_exprs_F)
dim(case_exprs_M)

# calculate 80th percentile for each sample in each group
...
plot_gender_specific_genes(expression_data_normalised_as_data_frame, gender_comparison[2], gene_list, Probe_Detection_Threshold, paste(dataset, "case_samples", sep="_"))
...
data_case_exprs_good_probes_M<-data_exprs_good_probes[,colnames(data_exprs_good_probes)%in%rownames(male_samples)]
data_case_exprs_good_probes_F<-data_exprs_good_probes[,colnames(data_exprs_good_probes)%in%rownames(female_samples)]
