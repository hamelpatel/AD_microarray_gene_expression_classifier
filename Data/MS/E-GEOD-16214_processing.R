...
# DATE: 12/05/2018
# ARRAY EXPRESS NUMBER: E-GEOD-16214
# DISORDER: Multiple Sclerosis
# MICROARRAY PLATFORM: Affy
# EXPRESSION CHIP: HG U133A plus2.0
# NUMBER OF SAMPLES: 
# TISSUE: Whole Blood
#
# NOTES - 
# single disease (no controls)
# # using non-treated indivdiduals
...
work_dir="/media/hamel/Workspace/Dropbox/Projects/AD-classification/1.Data/3.MS/E-GEOD-16214"
...
#raw data only
#data_raw=getAE("E-GEOD-16214", type = "raw")

#processed data only
#data_raw=getAE("E-GEOD-16214", type = "processed")

#all data
data_raw=https_getAE("E-GEOD-16214", type = "full")
...
dim(phenotype_data)
table(phenotype_data$Factor.Value.clinical.treatment.)



phenotype_data_subset<-subset(phenotype_data, Factor.Value.clinical.treatment. == "none")
dim(phenotype_data_subset)
table(phenotype_data_subset$Factor.Value.clinical.treatment.)
head(phenotype_data_subset)

table(phenotype_data_subset$Hybridization.Name)
...
expression_data_subset<-expression_data[,gsub(".CEL", "", colnames(exprs(expression_data))) %in% rownames(phenotype_data_subset)]
