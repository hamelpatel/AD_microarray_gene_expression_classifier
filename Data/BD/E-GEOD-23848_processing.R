...
# DATE: 09/05/2018
# ARRAY EXPRESS NUMBER: E-GEOD-23848
# DISORDER: Biolar
# MICROARRAY PLATFORM: Illumina
# EXPRESSION CHIP: human 6v2
# NUMBER OF SAMPLES: 
# TISSUE: Whole Blood
#
# NOTES - 
...
work_dir="/media/hamel/Workspace/Dropbox/Projects/AD-classification/1.Data/5.BD/E-GEOD-23848"
...
#raw data only
#data_raw=getAE("E-GEOD-23848", type = "raw")

#processed data only
#data_raw=getAE("E-GEOD-23848", type = "processed")

#all data
data_raw=https_getAE("E-GEOD-23848", type = "full")
...
GEO_expression_data <- getGEO("GSE23848", GSEMatrix =TRUE, getGPL=FALSE)
 if (length(GEO_expression_data) > 1) idx <- grep(GEO_expression_data@annotation, attr(GEO_expression_data, "names")) else idx <- 1
GEO_expression_data <- GEO_expression_data[[idx]]

expression_data

head(pData(GEO_expression_data))
#head(exprs(expression_data))[,1:5]

# download raw expression values and read (processed data only has 2839 genes)

setwd(raw_dir)

# need to change colnames
temp_expr<-read.csv("GSE23848_non-normalized_data.txt", sep="\t", check.names = F)
head(temp_expr)[,1:5]

# sample to columns
colnames(temp_expr)[grep("BP|HC", colnames(temp_expr))]<-paste("SAMPLE", colnames(temp_expr)[grep("BP|HC", colnames(temp_expr))])
colnames(temp_expr)

#save
write.table(temp_expr, file="temp_expr.txt", sep="\t", row.names=F, quote = F)

lumi_expression_data <- read.ilmn("temp_expr.txt", probeid="ProbeID", expr="SAMPLE " , other.columns="Detection Pval")

dim(lumi_expression_data)
head(lumi_expression_data$E)[,1:5]

expression_data_raw<-(neqc(lumi_expression_data, detection.p="Detection Pval"))$E
...
#expression_chip="illuminaHumanv2"
expression_chip="illuminaHumanv2BeadID"
#expression_chip="lumiHumanAll"
...
pheno_temp<-pData(GEO_expression_data)
rownames(pheno_temp)<-paste(rownames(pheno_temp), "_sample_table", sep="")

head(pheno_temp)
head(pData(expression_data))

phenotype_data_subset<-merge(pData(expression_data)[3], pheno_temp, by="row.names")
rownames(phenotype_data_subset)<-phenotype_data_subset$Row.names
head(phenotype_data_subset)

# move sample id to rwnames
rownames(phenotype_data_subset)<-phenotype_data_subset$Comment..Sample_description..1

head(phenotype_data_subset)
names(phenotype_data_subset)

# extract pheno of interest
Ethnicity<-phenotype_data_subset[16]
colnames(Ethnicity)<-"Ethnicity"

Tissue<-phenotype_data_subset[14]
colnames(Tissue)<-"Tissue"

Age<-phenotype_data_subset[17]
colnames(Age)<-"Age"

Diagnosis<-phenotype_data_subset[12]
colnames(Diagnosis)<-"Diagnosis"

Gender<-phenotype_data_subset[15]
...
Diagnosis[grep("Bipolar", Diagnosis[,1]),]<-"case"
table(Diagnosis)
...
ncol(expression_data_raw)==nrow(phenotype_data_subset)
all(colnames(expression_data_raw)%in%rownames(phenotype_data_subset))==T
...
boxplot(head(lumi_expression_data$E[,1:5]))
png(file="raw_data_boxplot.png")
boxplot(lumi_expression_data$E)
dev.off()

plotDensity(head(lumi_expression_data$E), logMode=F, addLegend=F)
png(file="raw_data_density_plot.png")
plotDensity(lumi_expression_data$E, logMode=F, addLegend=F)
dev.off()


##### PRE-PROCESS - RAW ######

#background correct - data already bgcorected

#expression_data_background_corrected<-mas5(expression_data_subset)

#add negative values to all, log2 and then normalise
...
# 
expression_data_normalised<-expression_data_raw
...
expression_data_normalised_as_data_frame<-as.data.frame(expression_data_normalised_as_data_frame)
...
#separae male/female IDs - use predicted - *used as boxplot verifies this to be true
...
#*** based on probe detection plots

XIST_gender<-t(expression_data_normalised_as_data_frame[rownames(expression_data_normalised_as_data_frame)=="3390521",])
XIST_gender

female_samples<-as.data.frame(XIST_gender[XIST_gender>8,])
female_samples

male_samples<-as.data.frame(XIST_gender[XIST_gender<8,])
male_samples

head(female_samples)
head(male_samples)

# changes predicted gender

XIST_gender<-as.data.frame(XIST_gender)
XIST_gender$XIST_Gender<-"Unknown"

XIST_gender$XIST_Gender<-as.character(XIST_gender$XIST_Gender)
XIST_gender[XIST_gender$`3390521`<8,2]<-"female"
XIST_gender[XIST_gender$`3390521`>8,2]<-"male"

XIST_gender

#merge

gender_comparison<-merge(gender_comparison, XIST_gender, by="row.names")
rownames(gender_comparison)<-gender_comparison$Row.names
gender_comparison$Row.names<-NULL
gender_comparison$`3390521`<-NULL
head(gender_comparison)

colnames(gender_comparison)[2]<-"Predicted_Gender"
...
# dev.off()
# 
# pdf("Probe_detection_threshold_based_on_XIST_gender_specific_and_house_keeping_genes.pdf", height=15, width = 12)
# plot_gender_specific_genes(case_exprs, gender_comparison[3], gene_list, Probe_Detection_Threshold, paste(dataset, "case_samples", sep="_"))
# plot_gender_specific_genes(control_exprs, gender_comparison[3], gene_list, Probe_Detection_Threshold, paste(dataset, "control_samples", sep="_"))
# dev.off()

#move XIST gender to predicted

gender_comparison$Predicted_Gender<-gender_comparison$XIST_Gender
gender_comparison$XIST_Gender<-NULL
