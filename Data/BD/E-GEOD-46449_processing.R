...
# DATE: 13/05/2018
# ARRAY EXPRESS NUMBER: E-GEOD-46449
# DISORDER: Bipolar Disorder
# MICROARRAY PLATFORM: AFFY
# EXPRESSION CHIP: HG U133A plus 2.0
# NUMBER OF SAMPLES: 
# TISSUE: Whole Blood
#
# NOTES - 
# keep only BP patients + biological replica 1
#
...
work_dir="/media/hamel/Workspace/Dropbox/Projects/AD-classification/1.Data/5.BD/E-GEOD-46449/"
...
#raw data only
#data_raw=getAE("E-GEOD-46449", type = "raw")

#processed data only
#data_raw=getAE("E-GEOD-46449", type = "processed")

#all data
data_raw=https_getAE("E-GEOD-46449", type = "full")

##### CREATE R EXPRESSION OBJECT #####

# METHOD 1 - convert MAGE-TAB files into expresssion set  - USING RAW DATA - uses with expression chip annotation files. using just to get pheno info
...
# expression_data <- getGEO("GSE63060", GSEMatrix =TRUE, getGPL=FALSE)
# if (length(expression_data) > 1) idx <- grep(expression_data@annotation, attr(expression_data, "names")) else idx <- 1
...
table(phenotype_data$FactorValue..GENOTYPE.)

table(phenotype_data$Comment..Sample_title.)

# subset to kepp only BP + biological replica 1

#phenotype_data_subset<-subset(phenotype_data, `status:ch1` %in% c("AD", "CTL"))
phenotype_data_subset<-phenotype_data[grep("biological replicate 1", phenotype_data$Comment..Sample_title.),]

table(phenotype_data_subset$Comment..Sample_title.)
table(phenotype_data_subset$FactorValue..GENOTYPE.)
...
Diagnosis<-phenotype_data_subset[5]
colnames(Diagnosis)<-"Diagnosis"

Gender<-phenotype_data_subset[10]
...
table(Diagnosis)
Diagnosis[grep("control subject", Diagnosis[,1]),]<-"control"
Diagnosis[grep("bipolar patient", Diagnosis[,1]),]<-"case"
table(Diagnosis)
...
table(Tissue)
Tissue[1]<-"leukocytes from whole blood"
table(Tissue)
...
dim(exprs(expression_data))
expression_data_subset<-expression_data[colnames(expression_data) %in% rownames(phenotype_data_subset)]
dim(exprs(expression_data_subset))
...
ncol(exprs(expression_data_subset))==nrow(phenotype_data_subset)
...
boxplot(head(exprs(expression_data_subset)))
png(file="raw_data_boxplot.png")
boxplot(exprs(expression_data_subset))
dev.off()

plotDensity(head(exprs(expression_data_subset)), logMode=F, addLegend=F)
png(file="raw_data_density_plot.png")
plotDensity(exprs(expression_data_subset), logMode=F, addLegend=F)
dev.off()
...
massi.select.out <-massi_select(expression_data_normalised_as_data_frame, y_chromo_probes, threshold=3)
...
# dip test <0.08 - sex bias - all males
...
# calculate percentile for each sample in each group
...
good_probe_list<-unique(sort(c(case_exprs_expressed_probes_list,
                               control_exprs_expressed_probes_list)))
...
data_case_exprs_good_probes_QC<-run_sample_network_plot(data_case_exprs_good_probes, sample_network_ZK_threshold)
data_control_exprs_good_probes_QC<-run_sample_network_plot(data_control_exprs_good_probes, sample_network_ZK_threshold)
...
all(rownames(data_case_exprs_good_probes_QC)==rownames(data_case_exprs_good_probes_QC))

# transform and cbind all dataframes

expression_data_QCd<-rbind(t(data_case_exprs_good_probes_QC),
                           t(data_control_exprs_good_probes_QC))
...
##### TEST - PROBE DETECTION THRESHOLD PLOTS AFTER QC #####
...
Gene_symbols_entrez <- mappedkeys(eval(parse(text = paste(expression_chip, "SYMBOL", sep=""))))

# Convert to a list
Gene_symbols_entrez <- merge(Gene_symbols, probe_entrez_mapping, by="probe_id")
head(Gene_symbols_entrez)

# extract HK 

#Expressed in females only
XIST_entrez_ID<-subset(Gene_symbols_entrez, symbol=="XIST")
XIST_entrez_ID

#Expressed in Males only
PRKY_entrez_ID<-subset(Gene_symbols_entrez, symbol=="PRKY")
PRKY_entrez_ID

RPS4Y1_entrez_ID<-subset(Gene_symbols_entrez, symbol=="RPS4Y1")
RPS4Y1_entrez_ID

KDM5D_entrez_ID<-subset(Gene_symbols_entrez, symbol=="KDM5D")
KDM5D_entrez_ID

USP9Y_entrez_ID<-subset(Gene_symbols_entrez, symbol=="USP9Y")
USP9Y_entrez_ID

UTY_entrez_ID<-subset(Gene_symbols_entrez, symbol=="UTY")
UTY_entrez_ID

# HK genes expressed in all cells + males + females
MKRN1_entrez_ID<-subset(Gene_symbols_entrez, symbol=="MKRN1")
MKRN1_entrez_ID

ADIPOR1_entrez_ID<-subset(Gene_symbols_entrez, symbol=="ADIPOR1")
ADIPOR1_entrez_ID

BNIP3L_entrez_ID<-subset(Gene_symbols_entrez, symbol=="BNIP3L")
BNIP3L_entrez_ID

#RNF10_entrez_ID<-subset(Gene_symbols_entrez, symbol=="RNF10")
#RNF10_entrez_ID

# merge all genes 
gene_entrez_list<-rbind(XIST_entrez_ID,
                        PRKY_entrez_ID,
                        RPS4Y1_entrez_ID,
                        KDM5D_entrez_ID,
                        USP9Y_entrez_ID,
...
head(gene_entrez_list)
gene_entrez_list$probe_id<-NULL
head(gene_entrez_list)
# make unique list
gene_entrez_list<-unique(gene_entrez_list)

#create new function to plot

plot_Gender_specific_genes_entrez<-function(Expression_table, Gender, genes_to_extract, Probe_Detection_Threshold, boxplot_title){
  #extract gene of interest
  expression_data_QCd_entrez_id_unique_pheno_gene_check<-expression_data_QCd_entrez_id_unique_pheno[,colnames(expression_data_QCd_entrez_id_unique_pheno)%in% genes_to_extract$entrezgene]
  # change colnames TO GENE SYMBOL using genes to extract file
  for (x in 1:dim(expression_data_QCd_entrez_id_unique_pheno_gene_check)[2]){
    colnames(expression_data_QCd_entrez_id_unique_pheno_gene_check)[x]<-genes_to_extract[genes_to_extract$entrezgene==colnames(expression_data_QCd_entrez_id_unique_pheno_gene_check)[x],1]
  }
  # add in Gender information
  expression_data_QCd_entrez_id_unique_pheno_gene_check_gender<-merge(Gender, expression_data_QCd_entrez_id_unique_pheno_gene_check, by="row.names")
  rownames(expression_data_QCd_entrez_id_unique_pheno_gene_check_gender)<-expression_data_QCd_entrez_id_unique_pheno_gene_check_gender$Row.names
  expression_data_QCd_entrez_id_unique_pheno_gene_check_gender$Row.names<-NULL
  #melt dataframe for plot
  expression_data_QCd_entrez_id_unique_pheno_gene_check_gender_melt<-melt(expression_data_QCd_entrez_id_unique_pheno_gene_check_gender, by="Predicted_Gender")
  # change variable colun from factor to character
  expression_data_QCd_entrez_id_unique_pheno_gene_check_gender_melt$variable<-as.character(expression_data_QCd_entrez_id_unique_pheno_gene_check_gender_melt$variable)
  # order dataframe by variable
  expression_data_QCd_entrez_id_unique_pheno_gene_check_gender_melt<-expression_data_QCd_entrez_id_unique_pheno_gene_check_gender_melt[order(expression_data_QCd_entrez_id_unique_pheno_gene_check_gender_melt$variable),]
  # calculate user defined percentie Probe_Detection_Threshold
  #sample_quantiles<-apply(expression_data_QCd_entrez_id_unique_pheno[7:ncol(expression_data_QCd_entrez_id_unique_pheno)], 2, quantile, probs=Probe_Detection_Threshold)
  # mean of used defined Probe_Detection_Threshold across samples
  mean_Probe_Detection_Threshold=Probe_Detection_Threshold
  #plot
  plot1<-qplot(variable, value, colour=get(colnames(Gender)), data = expression_data_QCd_entrez_id_unique_pheno_gene_check_gender_melt, geom = c("boxplot", "jitter")) + 
    geom_hline(yintercept = mean_Probe_Detection_Threshold) +
    ggtitle("boxplot_title") +
    theme(text = element_text(size=20), axis.text.x = element_text(angle=45, hjust=1)) +
    labs(x="Gene",y="Expression", colour = colnames(Gender)) 
  # 2nd legend
...
plot_Gender_specific_genes_entrez(expression_data_QCd_entrez_id_unique_pheno, gender_comparison[2], gene_entrez_list, 4.5, paste(dataset, "case_samples", sep="_"))

#plot using predicted gender
pdf("Probe_detection_threshold_based_on_PREDICTED_gender_specific_and_house_keeping_genes_TEST.pdf", height=15, width = 12)
plot_Gender_specific_genes_entrez(expression_data_QCd_entrez_id_unique_pheno, gender_comparison[2], gene_entrez_list, 4.5, paste(dataset, "case_samples", sep="_"))
dev.off()
