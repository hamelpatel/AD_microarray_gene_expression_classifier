...
# DATE: 21/05/2018
# ARRAY EXPRESS NUMBER: E-TABM-940
# DISORDER: ALS
...
work_dir="/media/hamel/Workspace/Dropbox/Projects/AD-classification/1.Data/9.ALS/E_TABM-940/"
...
#raw data only
#data_raw=getAE("E-TABM-940", type = "raw")

#processed data only
#data_raw=getAE("E-TABM-940", type = "processed")

#all data
data_raw=https_getAE("E-TABM-940", type = "full")
...
dim(phenotype_data)
table(phenotype_data$Factor.Value..DiseaseState.)

# subset - remove probable ALS
phenotype_data_subset<-subset(phenotype_data, `Factor.Value..DiseaseState.` %in% c("DEFINITIVE diagnosis of ALS", "NON ALS"))
dim(phenotype_data_subset)
table(phenotype_data_subset$Factor.Value..DiseaseState.)

#phenotype_data_subset<-phenotype_data

# extract pheno of interest
Ethnicity<-phenotype_data_subset[30]
...
Diagnosis<-phenotype_data_subset[29]
colnames(Diagnosis)<-"Diagnosis"

Gender<-phenotype_data_subset[7]
...
table(Diagnosis)
Diagnosis[grep("NON ALS", Diagnosis[,1]),]<-"control"
Diagnosis[grep("DEFINITIVE diagnosis of ALS", Diagnosis[,1]),]<-"case"
table(Diagnosis)
...
table(Gender)
#Gender[grep("0", Gender[,1]),]<-"female"
#Gender[grep("1", Gender[,1]),]<-"male"
#Gender[1]<-"Unknown"
table(Gender)
...
# susbet subjects of interest and convert to data.frame
expression_data_normalised_as_data_frame<-as.data.frame(expression_data_normalised[,colnames(expression_data_normalised) %in% rownames(phenotype_data_subset)])
...
table(expression_data_QCd_entrez_id_unique_pheno$Diagnosis, expression_data_QCd_entrez_id_unique_pheno$Predicted_Gender)
