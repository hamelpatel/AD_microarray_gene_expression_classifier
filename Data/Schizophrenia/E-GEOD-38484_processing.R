...
# DATE: 22/08/2018
# ARRAY EXPRESS NUMBER: E-GEOD-38484
...
#
# NOTES - 
#  mergeing all MS subgroups together
# lot of negative values after log2 transformation. added 15 to all samples after log2
...
work_dir="/media/hamel/Workspace/Dropbox/Projects/AD-classification/1.Data/4.Schizophrenia/E-GEOD-38484/"
...
#raw data only
# data_raw=https_getAE("E-GEOD-38484", type = "raw")

#processed data only
#data_raw=getAE("E-GEOD-38484", type = "processed")

#all data
#data_raw=https_getAE("E-GEOD-38484", type = "full")
...
#ubnable to process through array express. used GEO instead - using for phenot data only
expression_data <- getGEO("GSE38484", GSEMatrix =TRUE, getGPL=FALSE)
...
lumi_expression_data <- read.ilmn("GSE38484_non_normalized.txt.gz",probeid="ID_REF",expr="AVG_Signal.",other.columns="Detection.")
...
dim(phenotype_data)
table(phenotype_data$FactorValue..STATUS.)
...
Tissue<-phenotype_data_subset[8]
colnames(Tissue)<-"Tissue"

Age<-phenotype_data_subset[33]
colnames(Age)<-"Age"

Diagnosis<-phenotype_data_subset[35]
colnames(Diagnosis)<-"Diagnosis"

Gender<-phenotype_data_subset[34]
...
Diagnosis[grep("schizophrenia \\(SCZ\\)", Diagnosis[,1]),]<-"case"
table(Diagnosis)
...
# SUBSET EXPRESSION DATA AND MATCH TO PHENO TABLE #####
