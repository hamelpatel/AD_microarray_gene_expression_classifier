...
# DATE: 16/05/2018
# ARRAY EXPRESS NUMBER: E-GEOD-74143
# DISORDER: Rheumatoid Arthritis
# MICROARRAY PLATFORM: AFFY
# EXPRESSION CHIP: HG plus2.0 
# NUMBER OF SAMPLES: 
# TISSUE: Whole Blood
#
# NOTES - 
# no controls
# - using processed - cannot mas5 background correct this chip - 
#
# chip annotaiton file does not exist for R.3.4.4
# this chip has a few extra probes plus has _PM attached to all probesets. - removing _PM from all probes
...
work_dir="/media/hamel/Workspace/Dropbox/Projects/AD-classification/1.Data/7.Rheumatoid_Arthritis/E-GEOD-74143"
...
#raw data only
#data_raw=getAE("E-GEOD-74143", type = "raw")

#processed data only
#data_raw=getAE("E-GEOD-74143", type = "processed")

#all data
data_raw=https_getAE("E-GEOD-74143", type = "full")
...
table(phenotype_data$Characteristics..rheumatoid.factor.positivity.)
#table(phenotype_data$FactorValue..DIABETES.)
#table(phenotype_data$FactorValue..CAD., phenotype_data$FactorValue..DIABETES.)
...
Tissue<-phenotype_data[11]
colnames(Tissue)<-"Tissue"

Age<-phenotype_data[4]
colnames(Age)<-"Age"

Diagnosis<-phenotype_data[5]
colnames(Diagnosis)<-"Diagnosis"

Gender<-phenotype_data[15]
...
table(Diagnosis)
#Diagnosis[grep("No", Diagnosis[,1]),]<-"control"
Diagnosis[grep("rheumatoid arthritis", Diagnosis[,1]),]<-"case"
table(Diagnosis)
...
table(Gender)
#Gender[grep("m", Gender[,1]),]<-"male"
#Gender[grep("f", Gender[,1]),]<-"female"
#Gender[1]<-"Unknown"
table(Gender)
...
expression_data_normalised_as_data_frame<-as.data.frame(exprs(expression_data))

# modify data 

rownames(expression_data_normalised_as_data_frame)<-gsub("_PM","", rownames(expression_data_normalised_as_data_frame))
...
plot_gender_specific_genes(case_exprs, gender_comparison[2], gene_list, 0.7, paste(dataset, "case_samples", sep="_"))
...
data_case_exprs_good_probes_F_QC<-run_sample_network_plot(data_case_exprs_good_probes_F, -2.5)
...
cat("FULL Probe Numbers after QC:", dim(full_expression_data_QCd_entrez_id_unique_pheno)[2]-6, "\n", file="Summary_Report.txt", append=T)
...
# plot_Gender_specific_genes_entrez(expression_data_QCd_entrez_id_unique_pheno, gender_comparison[2], gene_entrez_list, 6, paste(dataset, "case_samples", sep="_"))
# 
